...
import { AvailableSlot, BookingConflictInfo, BookingWithResource } from "../app/modules/Booking/Booking.interface"
...
export function validateBookingDuration(startTime: Date, endTime: Date): { valid: boolean; message?: string } {
  ...
  if (durationMinutes < MIN_BOOKING_MINUTES) {
    return { valid: false, message: `Minimum booking duration is ${MIN_BOOKING_MINUTES} minutes` }
  }

  if (durationHours > MAX_BOOKING_HOURS) {
    return { valid: false, message: `Maximum booking duration is ${MAX_BOOKING_HOURS} hours` }
  }
...
export function checkBookingConflicts(
  ...
  for (const booking of existingBookings) {
    ...
    const bufferedStart = subtractBufferTime(new Date(booking.startTime), BUFFER_MINUTES)
    const bufferedEnd = addBufferTime(new Date(booking.endTime), BUFFER_MINUTES)
...
export function getBookingStatus(startTime: Date, endTime: Date): BookingStatus {
  const now = new Date()
...
export function findAvailableSlots(
  ...
  for (const booking of sortedBookings) {
    const bookingBufferedStart = subtractBufferTime(new Date(booking.startTime), BUFFER_MINUTES)
    
    // If there's a gap before this booking
    if (currentTime < bookingBufferedStart) {
      const slotDuration = (bookingBufferedStart.getTime() - currentTime.getTime()) / (1000 * 60)
    ...
    const bookingBufferedEnd = addBufferTime(new Date(booking.endTime), BUFFER_MINUTES)
    currentTime = new Date(Math.max(currentTime.getTime(), bookingBufferedEnd.getTime()))
  }
